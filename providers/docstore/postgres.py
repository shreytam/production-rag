...
class PostgresDocumentRegistry:
    ...
    def create(self, record: DocumentRecord) -> None:
        with self._conn() as c, c.cursor() as cur:
            cur.execute(
                ...
                f"content_type, size_bytes, status, blob_key, collection_id, error, chunk_count) "
                f"VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (document_id) DO NOTHING",
                [record.document_id, record.tenant_id, record.filename,
                 record.content_type, record.size_bytes, record.status.value,
                 record.blob_key, record.collection_id, record.error, record.chunk_count])
    ...
    def _row(row) -> DocumentRecord:
        return DocumentRecord(
            ...
            collection_id=row[7], error=row[8], chunk_count=row[9])
